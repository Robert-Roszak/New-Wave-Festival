...
class OrderTicketForm extends React.Component {
  ...
  submitForm = async (e) => {
    const { order } = this.state;
    const { addSeat, updateConcert} = this.props;

    e.preventDefault();

    if(order.client && order.email && order.day && order.seat) {
      await addSeat(order);
      await updateConcert(order);
      this.setState({ 
        order: {
          client: '',
          email: '',
      ...
      <Form className="order-ticket-form" onSubmit={submitForm}>
        <Row>
          <Col xs="12" md="6">
            ...
            <FormGroup check>
              <Label check>
                <Input required type="checkbox" /> I agree with <a href="/t&c">Terms and conditions</a> and <a href="/privacy">Privacy Policy</a>.
              </Label>
            </FormGroup>
